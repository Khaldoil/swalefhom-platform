...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Public can view published books'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'books' 
    AND policyname = 'Authenticated users can manage books'
  ) THEN
...
VALUES 
  ('كتب التراث', 'كتب متخصصة في التراث السعودي', 'book', 'story', 1),
  ('الدراسات التاريخية', 'كتب ودراسات في تاريخ المملكة', 'bookmark', 'story', 2),
  ('الفنون الشعبية', 'كتب عن الفنون والحرف التقليدية', 'book-open', 'story', 3),
  ('المعاجم والقواميس', 'معاجم وقواميس للألفاظ التراثية', 'book', 'story', 4)
